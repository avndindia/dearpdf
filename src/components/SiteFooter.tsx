...
export default function SiteFooter() {
  return (
    <footer className="mt-8 w-full border-t border-border-subtle bg-white shadow-[0_-1px_8px_rgba(0,0,0,0.02)] dark:border-[#1f2a3f] dark:bg-[#0a0e17] dark:shadow-none">
      <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-8 sm:px-6">
        <div className="flex flex-col items-start justify-between gap-4 rounded-2xl border border-emerald-200/60 bg-security-bg/70 p-4 md:flex-row md:items-center dark:border-emerald-800/50 dark:bg-emerald-950/40">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {["No cloud upload", "Works offline", "Free forever"].map((label) => (
              <span
                key={label}
                className="rounded-md border border-emerald-200 bg-white px-2.5 py-1 font-mono text-[12px] text-security-emerald shadow-sm dark:border-emerald-800/40 dark:bg-[#121826]"
              >
                {label}
              </span>
            ))}
        ...
        <div className="flex flex-col items-center justify-between gap-4 pt-1 md:flex-row">
          <div className="flex flex-wrap items-center justify-center gap-3 text-[13px] text-slate-500 dark:text-slate-400">
            ...
            <span className="rounded border border-slate-200 bg-slate-100 px-2.5 py-0.5 font-mono text-[12px] text-secondary dark:border-slate-800 dark:bg-[#151c2d] dark:text-sky-400">
              v2.5.0
            </span>
          </div>
